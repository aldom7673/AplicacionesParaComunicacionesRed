...
def ServirPorSiempre(socketTcp, numeroConexiones, barrier):
    try:
        while True:
            ...
            if( l == 0): # Se juega con el primer tablero que se haya creado
                ...
                for i in range(1, numeroConexiones + 1):
            ...
            threads = threading.Thread(target=RecibirTiros,args=(client_conn, client_addr, identificador,barrier,))
            threads.start()
            gestion_conexiones()
...
def RecibirTiros(conn, addr, identificador, barrier):
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as TCPServerSocket:
    ...
    ServirPorSiempre(TCPServerSocket, int(numConn), barrier)
