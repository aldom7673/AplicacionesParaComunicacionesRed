...
def RenombrarArchivo(ruta):    
    ...
    if(ruta in ArchivosAbiertos):
        input("\t El archivo esta abierto, no se puede renombrar. Pulsa enter para continuar ... ")
        return
    
    nuevoNombre = input( "\t Ingresa el nuevo nombre para '" + nombreArchivo + "': ")
    
    ArchivosAbiertos.append(ruta + "/" + nombreArchivo);
...
def ModificarArchivo(ruta):
    ...
    if(ruta in ArchivosAbiertos):
        input("\t El archivo esta abierto, no se puede modificar. Pulsa enter para continuar ... ")
        return
    ArchivosAbiertos.append(ruta + nombreArchivo)
    ...
    ArchivosAbiertos.remove(ruta + nombreArchivo)
...
def BorrarArchivo(ruta):
    ...
    if(ruta in ArchivosAbiertos):
        input("\t El archivo esta abierto, no se puede eliminar. Pulsa enter para continuar ... ")
        return
...
def BorrarDirectorio(ruta):
    ...
    for contenido in contenidos:
        if("." in contenido):
            if(not (ruta + "/" + contenido) in ArchivosAbiertos):
                os.remove(ruta + "/" + contenido)
        else:            
            BorrarDirectorio(ruta + "/" + contenido)
    
    if(not (ruta + "/" + contenido) in DirectoriosAbiertos):
        os.rmdir(ruta)
...
def RenombrarDirectorio(nuevoDirectorio, ruta):
    ...
    for contenido in contenidos:
        if("." in contenido):
            if(not (ruta + "/" + contenido) in ArchivosAbiertos):
                ArchivosAbiertos.append(ruta + "/" + contenido)
...
def AbrirArchivo(ruta):
    global ArchivosAbiertos
    nombreArchivo = SeleccionarArchivo(os.listdir(ruta), "abrir", False)
    ArchivosAbiertos.append(nombreArchivo)    
...
def AbrirDirectorio(ruta):
    ...
    if( directorio == ""):
        ...
        return ""
    DirectoriosAbiertos.append(ruta + directorio + "/")
    return directorio
...
def RegresarDirectorio(ruta):
    ...
    if( ruta.replace(HOME,"") != "/"):
        ...
        for i in range(0, len(arrayRuta) - 2):
            ...
            print(rActual)
        DirectoriosAbiertos.remove(ruta)
        return rActual
    return "/"
